...
(async () => {
  ...
  check('late request notified admin, HOD and asst HOD (Chef station sees it in Requests)', ['admin@paradisecoveresortfiji.com', 'hod.hk@paradisecoveresortfiji.com', 'asst.hk@paradisecoveresortfiji.com'].every(e => hodN.includes(e)), hodN.join(','));

  // ---------- 6. Chef: accept special + late, kitchen list, report + roster
  await login('chef', 'kitchen2026');
  await page.evaluate(() => v3RememberActor('Vikash Chand'));
  await nav('chef'); await shot('chef-home');
  ...
  await nav('meals'); check('Chef station cannot open personal Meals', await page.evaluate(() => state.tab !== 'meals'));
  ...
  check('users screen shows "x of N used" seat counts', /Admin\s+\d+ of 5 used/.test(seatTxt) && /Superadmin\s+\d+ of 3 used/.test(seatTxt) && !/Boat manager\s+\d+ of/.test(seatTxt) && !/Chef\s+\d+ of/.test(seatTxt), seatTxt.replace(/\n/g,' '));
  ...
  check('migration keeps HOD, boat work moves to the station', /staff,hod,boat_manager,boat_captain → HOD · Boat work → station login/.test(mg), mg.replace(/\n/g,' | ').slice(0,200));
  check('migration keeps assistant HOD flag, boat work → station', /staff,assistant_hod,boat_manager,boat_captain → Staff \+ assistant HOD flag · Boat work → station login/.test(mg));
  check('migration flags HOD in department "Other"', /Pranil Rama[\s\S]*department "Other"/.test(mg));
  check('migration shows admin/superadmin seats only', /of 3 used/.test(mg) && /of 5 used/.test(mg) && !/of 2 used/.test(mg));
  check('migration: nobody loses rights', /Nobody loses rights/.test(mg));
  check('migration deactivates the Test Test account (data kept)', /PROPOSED DEACTIVATION[\s\S]*paradisecove679@gmail\.com[\s\S]*data kept/i.test(mg));
  check('migration maps chef/captain → staff + station, assistant HOD → staff+flag', /boat_captain → Staff · Boat work → station login/.test(mg) && /chef → Staff · Chef work → station login/.test(mg) && /assistant_hod → Staff \+ assistant HOD flag/.test(mg));
