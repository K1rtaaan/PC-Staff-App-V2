...
function initializeSheets() {
  ...
  ensureSheet(ss, 'Users', [
    ...
    'role', 'roster', 'village', 'active', 'createdAt', 'verified'
  ]);
  ...
  ensureSheet(ss, 'Suggestions', [
    'id', 'userEmail', 'userName', 'title', 'body', 'votes', 'voters', 'createdAt', 'status'
  ]);
...
function placeDinnerOrder(p) {
  ...
  // upsert: one order per user per service date
...
function getDinnerOrders(p) {
  ...
  return { success: true, data: { orders: orders, cutoff: dinnerCutoffInfo() } };
}
...
function getKitchenDashboard(p) {
  ...
  var dinners = sheetToObjects('Dinner Orders').filter(function (o) {
    return String(o.serviceDate).indexOf(dinnerDate) === 0 && o.status !== 'cancelled';
  });
  ...
  return {
    ...
    data: {
      ...
      lunchDate: lunchDate,
      dinner: { orders: dinners, late: lateDinner, tally: tally(dinners), cutoff: dinnerInfo },
...
function addSuggestion(p) {
  ...
  appendRow('Suggestions', row, ['id', 'userEmail', 'userName', 'title', 'body', 'votes', 'voters', 'createdAt', 'status']);
...
function voteSuggestion(p) {
  ...
  if (!found) return { success: false, error: 'Not found' };
  var email = String(p.userEmail || p.requesterEmail || '').toLowerCase();
  var voters = String(found.voters || '').split(',').map(function (s) { return s.trim(); }).filter(Boolean);
  if (voters.indexOf(email) >= 0) return { success: false, error: 'Already voted' };
  voters.push(email);
  var votes = Number(found.votes || 0) + 1;
  updateRowById('Suggestions', found.id, { votes: votes, voters: voters.join(',') });
  return { success: true, data: { votes: votes } };
}
